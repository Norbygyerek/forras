...
public class TestExperiment2 {
	...
	public static void setExperiment(Robo roboControl) {
		...
		rlNumSteps = new int[TestExperiment2.kNumEpisodes];
		rlReturn = new double[TestExperiment2.kNumEpisodes];
		...
		for (int i = 0; i < TestExperiment2.kNumEpisodes; ++i) {
		    avgSteps += rlNumSteps[i];
		    avgReturn += rlReturn[i];
		}
		
		/*average steps and returns*/
		avgSteps /= (double)TestExperiment2.kNumEpisodes;
		avgReturn /= (double)TestExperiment2.kNumEpisodes;
		
		/*print out results*/
		System.out.println("Number of episodes: " + TestExperiment2.kNumEpisodes);
