...
import edu.bbte.agentEpsilonGreedy.agentLib.functionApproximators.CMAC.FeatureBasedFunctionApproximatorInterface;
import edu.bbte.agentEpsilonGreedy.agentLib.learningModules.LearningModuleInterface;
import edu.bbte.agentEpsilonGreedy.agentLib.normalizers.ObservationScalerInterface;
import edu.bbte.agentEpsilonGreedy.agentLib.normalizers.TaskSpecObservationNormalizer;
...
/**
 * Oct 26 2008.  Found a bug in this code.  You really can't have Lambda = 1.0 with 
 * this implementation unless you have an unlimited memory.  See, there are a fixed number
 * of trace positions, and you keep them finite by removing ones under a threshold.
 * 
 * With Lambda=1.0, they are always all the same and the system gets a bit pooched.
 * 
 * One solution to this problem is to not use Lambda=1.0.  We could complain if they
 * pick that value.  Another is to print a warning and use 1.0-epsilon, and be principled
 * about actually using the system epsilon.  I think that's what I'll do.  Turns out you can't 
 * do 1.0-Double.minValue because 1.0 * (1.0-Double.minValue) == 1.0 (the exponent is so close to 
 * 1 that it doesn't decay.  Experimentally 1.0d-1.0e-15; seems to be a good value.
 * 
 * Do that, blow out old experiments, re-run.
 * 
 * Also check out taking out static initializers and cleaning up those memory bits
 * when cleanup is called.
 * 
 *  System.out.println(Double.MIN_VALUE); is that value.. Make sure times by it makes 
 * a number smaller.
 * @author btanner
 */
