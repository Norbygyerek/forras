    ...
    private void learnFromLesson(Lesson theLesson) {
        for (int t = theLesson.size() - 1; t >= 0; t--) {
            ...
            if (discardNonPolicyActions) {
                double[] actionProbs = theActionSelector.getActionProbabilities(thisStep.s, theLearningModule);
                if (actionProbs[thisStep.action] <= .1d) {
                    continue;
                }
            }
            ...
            if (thisStep.sprime != null) {
                //We'll get the next state value by sampling an action
                int sampledNextAction = theActionSelector.sampleAction(thisStep.sprime, theLearningModule);
                nextValue = theLearningModule.query(thisStep.sprime, sampledNextAction);
            }

            double thisValue = theLearningModule.query(thisStep.s, thisStep.action);
            double r = thisStep.reward;

            double target = r + nextValue;
            double delta = target - thisValue;

            theLearningModule.update(thisStep.s, thisStep.action, delta);
        }
    }
